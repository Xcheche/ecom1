...
class Product(models.Model):
    ...
    short_description = models.CharField(max_length=100,blank=True,help_text='brief description of the product..')
    description = models.TextField(max_length=700,blank=True,help_text='describe the product..') 
    additional_info = models.TextField(max_length=1000,blank=True,help_text='additional info of the product..')
    ...
    def new(self):
        if not self.new_since:
            return False
        return timezone.now() - self.new_since < timezone.timedelta(days=30)  # new = created within last 30 days
    ...
    def is_on_sale(self):
        ...
        now = timezone.now()
        return self.sale_start and self.sale_end and self.sale_start <= now <= self.sale_end
